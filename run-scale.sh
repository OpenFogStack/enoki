...
if [ -z "$FUNCTION" ] || [ -z "$PARAMETER" ] || [ -z "$THREADS" ] || [ -z "$LOAD_TIME" ] || [ -z "$LOAD_THREADS" ] || [ -z "$OUTPUT_FILE" ] || [ -z "$DELAY" ] || [ -z "$BANDWIDTH_MBPS" ] || [ -z "$DELAY_CLIENT_EDGE" ] || [ -z "$BANDWIDTH_CLIENT_EDGE_MBPS" ] || [ -z "$DEPLOYMENT_MODE" ] || [ -z "$TIMEOUT" ]; then
    echo "missing parameters"
    echo "usage: ./run-scale.sh <function> <parameter> <threads> <load_time> <load_threads> <output_file> <delay> <bandwidth_mbps> <delay_client_edge> <bandwidth_client_edge_mbps> <deployment_mode> <timeout>"
    exit 1
fi
